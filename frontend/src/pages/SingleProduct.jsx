...
const SingleProduct = () => {
  ...
  if (images.length > 0) {
    const img = images[selectedImageIndex] || images[0];

    if (img.startsWith("http")) {
      mainImage = img; // Cloudinary
    } else {
      mainImage = `${import.meta.env.VITE_BASEURL}/uploads/${img}`; // old uploads
    }
  }
    ...
    <div className="single-product">
      <div className="single-product-image">
        ...
        {images.length > 1 && (
          <div className="thumbnail-gallery">
            {images.map((img, index) => {
              let thumbUrl = "/no-image.png";

              if (img.startsWith("http")) {
                thumbUrl = img;
              } else {
                thumbUrl = `${import.meta.env.VITE_BASEURL}/uploads/${img}`;
              }
